...
for(i in 1:length(crop.types)){
  ...
  tmp_mean = apply(X = tmp, MARGIN = 1:2, FUN = mean, na.rm = T)
  assign(x = paste0('POD3_FBB_', crop.types[i]), value = tmp_mean)
}

for(i in 1:length(crop.types)) plot.field(get(paste0('POD3_FBB_', crop.types[i])), lon, lat, type='def', zlim=c(0,25))

harvesting_date = prescribed_harvesting_date_Sack[,,subset_index]

save(list = c('lon', 'lat', 'crop.types', 'POD3_FBB_Maize', 'POD3_FBB_Wheat', 'POD3_FBB_Soybean', 'POD3_FBB_Rice', 'harvesting_date'), file = 'POD3_FBB.RData')
