...
setwd('~/Desktop/0828 debug/analysis/data/crop/RY/')

for(method in c('POD3_FBB', 'POD3_DOSE', 'AOT40', 'W126')){
  load(paste0('ry_', method, '.RData'))
  yl.avg = array(NaN, dim=c(4,4))
  yl.sd = array(NaN, dim=c(4,4))
  
  for(icrop in 1:length(crop.types))
    for(iregion in 1:length(regions)){
      tmp.avg = mean( 100 *(1 - get(paste0('ry_', crop.types[icrop], '_', method))*get(paste0('mask_', crop.types[icrop]))*get(paste0('mask_', regions[iregion]))), na.rm = T) # yield loss
      #tmp.avg = mean(get(paste0('ry_', crop.types[icrop], '_', method))*get(paste0('mask_', crop.types[icrop]))*get(paste0('mask_', regions[iregion])), na.rm = T)
      tmp.sd = sd( 100 *(1 - get(paste0('ry_', crop.types[icrop], '_', method))*get(paste0('mask_', crop.types[icrop]))*get(paste0('mask_', regions[iregion]))), na.rm = T)
      #tmp.sd = sd(get(paste0('ry_', crop.types[icrop], '_', method))*get(paste0('mask_', crop.types[icrop]))*get(paste0('mask_', regions[iregion])), na.rm = T)
      #print(paste0(method, ': Relative yield for ', crop.types[icrop], ' in ', regions[iregion], ' is ', format(tmp.avg, digits = 3)))
      #tmp = 100 * (1-tmp)
      yl.avg[icrop, iregion] = tmp.avg
      yl.sd[icrop, iregion] = tmp.sd
    }
  ...
  yield.loss.sd = as.vector(yl.sd)
  assign(x = paste0('df_',method), value = data.frame(crops, region, method = method, yield.loss.avg, yield.loss.sd))
}
...
for(icrop in 1:length(crop.types))
  for(iregion in 1:length(regions)){
    tmp.avg = mean(100*(1-get(group[icrop])*get(paste0('mask_', crop.types[icrop]))*get(paste0('mask_', regions[iregion]))), na.rm = T)
    #tmp.avg = mean(get(group[icrop])*get(paste0('mask_', crop.types[icrop]))*get(paste0('mask_', regions[iregion])), na.rm = T)
    tmp.sd = sd(100*(1-get(group[icrop])*get(paste0('mask_', crop.types[icrop]))*get(paste0('mask_', regions[iregion]))), na.rm = T)
...
yield.loss.avg = as.vector(yl.avg)
yield.loss.sd = as.vector(yl.sd)
assign(x = 'df_m', value = data.frame(crops, region, method = 'M7/M12', yield.loss.avg, yield.loss.sd))
...
p = ggplot(df_all, aes(fill = method, y = yield.loss.avg, x = region)) +
  ...
  #geom_errorbar(aes(ymin=yield.loss.avg-yield.loss.sd, ymax=yield.loss.avg+yield.loss.sd), width=.2,
  #              position=position_dodge(.9)) +
  facet_wrap(~crops) +
  #coord_cartesian(ylim=c(0.8,1)) +
  theme(legend.title = element_blank()) +
  labs(x = 'Regions', y = 'Yield loss (%)', title = 'Regional average')
  #labs(x = 'Regions', y = 'Relative yield', title = 'Regional average')
print(p)
