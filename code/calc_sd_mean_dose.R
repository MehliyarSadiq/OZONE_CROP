...
for(i in 1:length(crop.types)){
  ...
  assign(x = paste0('mean_POD3_DOSE_', crop.types[i]), value = tmp_mean)
  assign(x = paste0('sd_POD3_DOSE_', crop.types[i]), value = tmp_sd)
}

for(i in 1:length(crop.types))
  plot.field(100*get(paste0('mask_', crop.types[i]))*get(paste0('sd_POD3_DOSE_', crop.types[i]))/get(paste0('mean_POD3_DOSE_', crop.types[i])), lon, lat, type='def', zlim=c(0,30))

harvesting_date = prescribed_harvesting_date_Sack[,,subset_index]

save(list = c('lon', 'lat', 'year_seq', 'crop.types', 'mean_POD3_DOSE_Maize', 'mean_POD3_DOSE_Wheat', 'mean_POD3_DOSE_Soybean', 'mean_POD3_DOSE_Rice', 'sd_POD3_DOSE_Maize', 'sd_POD3_DOSE_Wheat', 'sd_POD3_DOSE_Soybean', 'sd_POD3_DOSE_Rice', 'harvesting_date'), file = 'mean_sd_POD3_DOSE.RData')
