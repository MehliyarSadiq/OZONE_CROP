...
plot.field(tmp[6:144,17:84]*1e-4, lon[6:144], lat[17:84], col = tim.colors(n=32)[17:32], type = 'def', zlim=c(0,20), legend.only = T)
dev.off()
...
for(crop.type in crop.types){
  ...
  pdf(paste0('yield_', crop.type,'.pdf'), width = 14, height = 8.5)
    plot.field(tmp[6:144,17:84]*1e-4, lon[6:144], lat[17:84], col = tim.colors(n=32)[17:32], type = 'def', zlim=c(0,20), image.only = T)
  dev.off()
}